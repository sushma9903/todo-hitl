...
def create_agent_graph(llm_with_tools, tools):
    ...
    def extract_todos(state:AgentState):
        """Pull todos generated by TodoListMiddleware into graph state.
        """
        todos = state.get("todos")

        #if no tools were generated, auto-approve
        if not todos:
            return {"approval_status": "approved"}
        
        return {"approval_status": "pending"}
    
    def plan_review_node(state: AgentState):
        todos = state.get("todos", [])
        plan_text="\n".join(
            f"- {item.get('task', str(item))}" for item in todos
        )
    ...
    def approval_decision_node(state: AgentState):
        last = state["messages"][-1].content.lower()
        if "yes" in last:
            return {"approval_status": "approval"}
        if "no" in last:
            return {"approval_status": "rejected"}
        # unclear response, then stay in approval loop
        return {}
    
    def approval_router(State:AgentState):
        status=status.get("approval_status")
        if status=="approved":
            return "tools"
        if status=="rejected":
            return END
    ...
    workflow.add_node("extract_todos", extract_todos)
    ...
    workflow.add_edge("agent", "extract_todos")
    workflow.add_edge("extract_todos", "plan_review")
    workflow.add_edge("plan_review", "approval_decision")
...
Always provide clear, concise answers based on the information available.""")
